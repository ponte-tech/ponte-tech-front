...
export default function EditarClientePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadCliente = async () => {
      try {
        ...
        setError(error.response?.data?.message || "Erro ao carregar cliente");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSend: UpdateClienteRequest = {
        ...formData,
      ...
      setError(error.response?.data?.message || error.message || "Erro ao atualizar cliente");
    } finally {
      setSaving(false);
    }
  };
      ...
      <Box sx={{ display: "flex", alignItems: "center", mb: 4 }}>
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Atualize os dados do cliente {cliente?.nome_fantasia}
          </Typography>
        </Box>
      </Box>
